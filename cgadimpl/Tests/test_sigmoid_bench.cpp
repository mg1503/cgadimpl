...
int main() {
    std::cout << "\n--- Sigmoid  Benchmark (Latency, Throughput, Accuracy) ---\n";
    auto run_relu_benchmark = [&](int64_t N, int iters = 15) {
        std::cout << "\n[N=" << N << "] Benchmarking Sigmoid ...\n";
        ...
        // 2. Accuracy Check
        Value L_ag = ag::sigmoid(X_bench);
        torch::Tensor x_torch = torch::from_blob(x_ptr, {N}, torch::kFloat32);
        torch::Tensor y_torch_ref = torch::sigmoid(x_torch);
        ...
        for (int i = 0; i < iters; ++i) {
            auto t0 = std::chrono::high_resolution_clock::now();
            torch::Tensor tmp = torch::sigmoid(x_torch);
            auto t1 = std::chrono::high_resolution_clock::now();
