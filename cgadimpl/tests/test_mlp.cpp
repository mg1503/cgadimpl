...
// int main() {
//     // ... (Data creation is correct) ...
//     const int B = 4, In = 8, H1 = 32, H2 = 32, H3 = 16, Out = 10;
//     Tensor Xt = Tensor::randn(Shape{{B, In}}, TensorOptions().with_req_grad(false));
//     Value  X  = make_tensor(Xt, "X");
//     Tensor Yt(Shape{{B, Out}}, false);
//     // ... (Yt initialization is correct) ...
//     Value Y = make_tensor(Yt, "Y");

//     // --- FIX: Change weight shapes to [out_features, in_features] ---
//     auto W1 = make_tensor(Tensor::randn(Shape{{H1, In}}, TensorOptions().with_req_grad(true)), "W1");
//     auto b1 = make_tensor(Tensor::zeros(Shape{{1, H1}}, TensorOptions().with_req_grad(true)), "b1");

//     auto W2 = make_tensor(Tensor::randn(Shape{{H2, H1}}, TensorOptions().with_req_grad(true)), "W2");
//     auto b2 = make_tensor(Tensor::zeros(Shape{{1, H2}}, TensorOptions().with_req_grad(true)), "b2");

//     auto W3 = make_tensor(Tensor::randn(Shape{{H3, H2}}, TensorOptions().with_req_grad(true)), "W3");
//     auto b3 = make_tensor(Tensor::zeros(Shape{{1, H3}}, TensorOptions().with_req_grad(true)), "b3");

//     auto W4 = make_tensor(Tensor::randn(Shape{{Out, H3}}, TensorOptions().with_req_grad(true)), "W4");
//     auto b4 = make_tensor(Tensor::zeros(Shape{{1, Out}}, TensorOptions().with_req_grad(true)), "b4");
//     // --- END FIX ---

//     // The forward pass now works correctly with the framework's 'linear' op
//     Value L1 = ag::silu(ag::linear(X,  W1, b1));
//     Value L2 = ag::gelu(ag::linear(L1, W2, b2));
//     Value L3 = ag::leaky_relu(ag::linear(L2, W3, b3));
//     Value logits = ag::linear(L3, W4, b4);

//     Value loss = ag::mse_loss(logits, Y);
//     backward(loss);

//     debug::print_value("loss", loss);
//     debug::print_grad("W1", W1);
    
//     std::cout << "\n✅ test_mlp completed successfully.\n";
//     return 0;
// }
#include "ad/ag_all.hpp"
#include "ad/debug.hpp"
