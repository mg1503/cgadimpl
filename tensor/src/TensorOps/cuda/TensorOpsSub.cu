// ===================================================================
// file: tensor/src/TensorOps/cuda/TensorOpsSub.cu
// ===================================================================
#ifdef WITH_CUDA
...
template<typename T>
    __global__ void sub_kernel_broadcast(const T* a, const T* b, T* output, 
    ...
    template <>
    __global__ void sub_kernel_broadcast<__half>(const __half* a, const __half* b, __half* output, 
                                               size_t a_rows, size_t a_cols,
                                               size_t b_rows, size_t b_cols,
                                               size_t out_rows, size_t out_cols)
    {
        ...
        if (idx < total_elems) {
            ...
            output[idx] = __hsub(a[a_idx], b[b_idx]);
        }
    }

    template <>
    __global__ void sub_kernel_broadcast<__nv_bfloat16>(const __nv_bfloat16* a, const __nv_bfloat16* b, __nv_bfloat16* output, 
                                                      size_t a_rows, size_t a_cols,
                                                      size_t b_rows, size_t b_cols,
                                                      size_t out_rows, size_t out_cols)
    {
...
template <typename T>
    ...
    void cuda_sub_tensor_inplace(Tensor& A, const Tensor& B, cudaStream_t stream)
    {
       ...
       // std::cout << "Subtraction Inplace CUDA\n";
